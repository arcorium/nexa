...
func getData() ([]*authZv1.CreatePermissionRequest, []*authZv1.CreatePermissionRequest) {
  // Seed role
  superPerms := sharedUtil.CastSlice(REACTION_SUPER_PERMS, func(action types.Action) *authZv1.CreatePermissionRequest {
  ...
  defaultPerms := sharedUtil.CastSlice(REACTION_DEFAULT_PERMS, func(action types.Action) *authZv1.CreatePermissionRequest {
    return &authZv1.CreatePermissionRequest{
      Resource: constant.SERVICE_RESOURCE,
      Action:   action.String(),
    }
