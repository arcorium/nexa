  ...
  tests := []struct {
  ...
  }{
    {
      ...
      setup: func(mocked *profileMocked, arg any, want any) {
        args := arg.(*args)
        expected := want.([]dto2.ProfileResponseDTO)

        userIds := sharedUtil.CastSlice(args.userIds, sharedUtil.ToAny[types.Id])

        result := sharedUtil.CastSliceP(expected, func(from *dto2.ProfileResponseDTO) entity.Profile {
    ...
    },
    {
      ...
      want: sharedUtil.GenerateMultiple(2, func() dto2.ProfileResponseDTO {
        return dto2.ProfileResponseDTO{
          Id:        types.MustCreateId(),
          FirstName: gofakeit.FirstName(),
          LastName:  gofakeit.LastName(),
          Bio:       gofakeit.LoremIpsumParagraph(1, 3, 20, "."),
...
func Test_profileService_UpdateAvatar(t *testing.T) {
  type args struct {
    ctx   context.Context
    input *dto2.ProfileAvatarUpdateDTO
  }
  tests := []struct {
    name  string
    setup setupProfileTestFunc
  ...
  }{
    {
      ...
      setup: func(mocked *profileMocked, arg any, want any) {
        ...
        mocked.Storage.EXPECT().
            UploadProfileImage(mock.Anything, &dto2.UploadImageDTO{
