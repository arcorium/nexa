...
func (u userService) ResetPassword(ctx context.Context, input *dto.ResetUserPasswordDTO) status.Object {
  ...
  if !input.Token.HasValue() {
    ...
    if !input.UserId.HasValue() {
      err = errors.New("expected user id, when there are no token provided")
      spanUtil.RecordError(err, span)
