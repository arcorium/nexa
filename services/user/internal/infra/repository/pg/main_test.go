...
func seed() error {
  profileModels := util.CastSlice(Profiles, func(from *entity.Profile) model.Profile {
    return model.FromProfileDomain(from, func(profile *entity.Profile, db *model.Profile) {
      db.UpdatedAt = time.Now()
    })
  })

  userModels := util.CastSlice(Users, func(from *entity.User) model.User {
