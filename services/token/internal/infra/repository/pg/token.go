...
func (t *tokenRepository) Upsert(ctx context.Context, token *entity.Token) error {
  ctx, span := t.tracer.Start(ctx, "TokenRepository.Upsert")
  defer span.End()
  ...
  res, err := t.db.NewInsert().
    ...
    Set("token = EXCLUDED.token, expired_at = EXCLUDED.expired_at, created_at = EXCLUDED.created_at").
    Exec(ctx)
