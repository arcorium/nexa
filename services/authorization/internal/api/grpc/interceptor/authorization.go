...
func Auth(claims *sharedJwt.UserClaims, meta interceptors.CallMeta) bool {
  ...
  case authZv1.RoleService_Update_FullMethodName:
    return authUtil.ContainsPermissions(claims.Roles, constant.AUTHZ_PERMISSIONS[constant.AUTHZ_UPDATE_ROLE])
  case authZv1.RoleService_Delete_FullMethodName:
    return authUtil.ContainsPermissions(claims.Roles, constant.AUTHZ_PERMISSIONS[constant.AUTHZ_DELETE_ROLE])
  case authZv1.RoleService_Find_FullMethodName:
    fallthrough
  case authZv1.RoleService_FindAll_FullMethodName:
    return authUtil.ContainsPermissions(claims.Roles, constant.AUTHZ_PERMISSIONS[constant.AUTHZ_READ_ROLE])
  case authZv1.RoleService_AddUser_FullMethodName:
    fallthrough
  case authZv1.RoleService_RemoveUser_FullMethodName:
    return authUtil.ContainsPermissions(claims.Roles, constant.AUTHZ_PERMISSIONS[constant.AUTHZ_MODIFY_USER_ROLE])
